...
class ExmoAPIUserStreamDataSource(UserStreamTrackerDataSource):
    MAX_RETRIES = 20
    ...
    async def _subscribe_to_channels(self, ws: websockets.WebSocketClientProtocol):
        ...
        try:
            ...
            self.logger().error("Error occured during subscribing to Exmo private channels.", exc_info=True)
            raise
        ...
        *required
        ...
        while True:
            try:
                ...
                self.logger().error(
                    "Unexpected error with Exmo WebSocket connection. Retrying after 30 seconds...",
                    exc_info=True
                )
